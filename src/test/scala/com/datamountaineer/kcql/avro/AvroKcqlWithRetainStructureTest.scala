...
class AvroKcqlWithRetainStructureTest extends WordSpec with Matchers {
  ...
  "AvroKcqlExtractor" should {
    "handle null payload" in {
      null.asInstanceOf[GenericContainer].kcql("SELECT * FROM topic  withstructure") shouldBe null.asInstanceOf[Any]
    }
    ...
    "throw an exception when the parameter is not a GenericRecord or NonRecordContainer" in {
      intercept[IllegalArgumentException] {
        ...
        }.kcql("SELECT * FROM topic  withstructure")
      }
    }
